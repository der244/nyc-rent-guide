  ...
  return <div className="space-y-6">
      ...
      <div className="print-only hidden print:block">
        <div className="print-header">
          <h1>NYC Rent Stabilized Renewal Calculation</h1>
          <div className="print-tenant-info">
            <strong>Generated:</strong> {new Date().toLocaleDateString()} | <strong>Source:</strong> nyclmg.com | <strong>RGB Order:</strong> #{scenarios.orderNumber}
          </div>
        </div>

        <div className="print-section">
          <div className="print-tenant-info">
            {inputs.tenantName && <><strong>Tenant:</strong> {inputs.tenantName} | </>}
            {inputs.address && <><strong>Property:</strong> {inputs.address} | </>}
            {inputs.unit && <><strong>Unit:</strong> {inputs.unit} | </>}
            <strong>Lease Start:</strong> {inputs.leaseStartDate.toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' })} | 
            <strong>Effective:</strong> {new Date(scenarios.effectivePeriod.split(' to ')[0]).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' })} to {new Date(scenarios.effectivePeriod.split(' to ')[1]).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' })}
          </div>
        </div>

        <div className="print-info-box">
          <strong>Current Rent:</strong> {inputs.preferentialRent ? `Legal ${formatCurrency(inputs.currentRent)} / Preferential ${formatCurrency(inputs.preferentialRent)}` : `Legal ${formatCurrency(inputs.currentRent)}`} | 
          <strong>1-Year End:</strong> {new Date(inputs.leaseStartDate.getFullYear() + 1, inputs.leaseStartDate.getMonth(), inputs.leaseStartDate.getDate() - 1).toLocaleDateString("en-US", { month: "2-digit", day: "2-digit", year: "numeric" })} | 
          <strong>2-Year End:</strong> {new Date(inputs.leaseStartDate.getFullYear() + 2, inputs.leaseStartDate.getMonth(), inputs.leaseStartDate.getDate() - 1).toLocaleDateString("en-US", { month: "2-digit", day: "2-digit", year: "numeric" })}
        </div>
        ...
        <table className="print-table">
          <thead>
            <tr>
              <th>Current Rent</th>
              <th>1-Year Lease Option</th>
              <th>2-Year Lease Option</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>
                <div className="print-amount">{formatCurrency(inputs.currentRent)}</div>
                <div className="print-calculation">Legal Regulated Rent</div>
              </td>
              <td style={{ textAlign: 'center' }}>
                ...
                <div className="print-calculation">
                  {scenarios.oneYear?.increases.length === 2 
            ...
            {inputs.preferentialRent && (
              <tr>
                <td>
                  <div className="print-amount">{formatCurrency(inputs.preferentialRent)}</div>
                  <div className="print-calculation">Tenant Currently Pays</div>
                </td>
                <td style={{ textAlign: 'center' }}>
                  {scenarios.oneYear?.preferentialResult && (
                    <>
                      ...
                      <div className="print-calculation">
                        +{formatCurrency((scenarios.oneYear.preferentialResult?.newTenantPay || 0) - (inputs.preferentialRent || 0))}
                      </div>
                    </>
                  )}
                </td>
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <Button variant="ghost" size="sm" className="h-4 w-4 p-0 hover:bg-muted text-muted-foreground hover:text-foreground" onClick={() => copyLeaseAmount(scenarios.twoYear.newLegalRent, '2-year final')} title="Copy final amount">
                <Copy className="h-2 w-2" />
                   ...
                   {/* Preferential rent row if applicable - Enhanced Color Scheme */}
                   {inputs.preferentialRent && scenarios.oneYear?.preferentialResult && scenarios.twoYear?.preferentialResult && <TableRow className="bg-slate-50 border-b-2 border-slate-300">
         ...
         <div className="flex items-center justify-center gap-2">
           ...
           <Button variant="ghost" size="sm" className="h-4 w-4 p-0 hover:bg-muted text-muted-foreground hover:text-foreground" onClick={() => copyLeaseAmount(inputs.preferentialRent!, 'current preferential rent')} title="Copy current preferential rent">
             <Copy className="h-2 w-2" />
                         ...
                         <div className="text-xs md:text-sm text-slate-600 font-semibold text-center">Preferential Rent (Tenant Pays)</div>
                       </TableCell>
                      <TableCell className="text-center space-y-1 px-2 sm:px-4">
                         {scenarios.oneYear?.increases.length === 2 ? <div className="flex items-center justify-center gap-2 relative">
             <div className="flex items-center justify-center gap-2">
               ...
               <Button variant="ghost" size="sm" className="h-4 w-4 p-0 hover:bg-muted text-muted-foreground hover:text-foreground" onClick={() => copyLeaseAmount(scenarios.oneYear.increases[0].newRent === inputs.preferentialRent ? inputs.preferentialRent! : inputs.preferentialRent! * (1 + scenarios.oneYear.increases[0].percentIncrease / 100), '1-year preferential year 1')} title="Copy Year 1 preferential amount">
                 <Copy className="h-2 w-2" />
                             ...
                             <div className="text-sm sm:text-lg font-bold text-slate-700">/</div>
             <div className="flex items-center justify-center gap-2">
               <div className="text-sm sm:text-lg font-bold text-slate-700">
                 {formatCurrency(scenarios.oneYear.preferentialResult.newTenantPay)}
               </div>
               <Button variant="ghost" size="sm" className="h-4 w-4 p-0 hover:bg-muted text-muted-foreground hover:text-foreground" onClick={() => copyLeaseAmount(scenarios.oneYear.preferentialResult.newTenantPay, '1-year preferential final')} title="Copy final preferential amount">
                 <Copy className="h-2 w-2" />
                         ...
                         {scenarios.oneYear?.increases.length === 2 && <div className="text-xs md:text-sm text-slate-600 italic">
                             {scenarios.oneYear.increases[0].period} / {scenarios.oneYear.increases[1].period}
                           </div>}
                      </TableCell>
                      <TableCell className="text-center space-y-1 px-2 sm:px-4">
